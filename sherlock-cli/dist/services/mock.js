...
const MOCK_EVENTS = [
    ...
    },
    {
        ...
        event: {
            ...
            data: {
                hypothesis: "Race condition introduced during async checkout refactor — inventory fetch is not awaited before decrement, returning a Promise instead of inventory data.",
                evidence: [
...
export async function* mockInvestigate(incidentId) {
    for (const item of MOCK_EVENTS) {
        await delay(item.delayMs);
        const event = { ...item.event };
        if (event.agent_name === "pipeline" && event.data)
...
export function mockIncidentState(incidentId) {
    return {
        ...
        root_cause: {
            ...
            root_cause: "Race condition in inventory decrement: async fetch result not awaited.",
            suspect_files: [
                {
...
export function mockPostmortem(incidentId) {

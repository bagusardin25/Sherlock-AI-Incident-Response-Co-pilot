...
import { mockInvestigate } from "../services/mock.js";
...
function renderEvent(event, timings) {
    ...
    if (event.status === "completed" && event.data) {
        ...
        switch (event.agent_name) {
            ...
            case "forensics":
                ...
                if (d.suspect_commit)
                    log.kv("Suspect Commit", chalk.yellow(d.suspect_commit));
                break;
            case "bob_analyst":
            ...
            case "fix":
                ...
                if (d.has_test)
                    log.success("Test case included");
                if (d.patch) {
            ...
            case "pipeline":
                ...
                if (d.incident_id)
                    log.kv("Incident ID", d.incident_id);
                break;
        }
    }
}
function showMetrics(totalStart, timings) {
    ...
    if (rcTime)
        console.log(chalk.cyan.bold("  │") + `  Root cause identified in: ${chalk.green.bold((rcTime / 1000).toFixed(1) + "s")}`.padEnd(49) + chalk.cyan.bold("│"));
    if (fixTime)
        console.log(chalk.cyan.bold("  │") + `  Patch generated in: ${chalk.green.bold((fixTime / 1000).toFixed(1) + "s")}`.padEnd(49) + chalk.cyan.bold("│"));
...
export const investigateCommand = new Command("investigate")
    ...
    if (mock)
        log.info("Backend unavailable — using mock mode");
    ...
    if (mock) {
        ...
        for await (const event of mockInvestigate(incidentId)) {
            if (event.status === "running") {
                ...
                if (start)
                    timings.set(event.agent_name, Date.now() - start);
                spinner.stop();
...
function streamSSE(url, spinner, timings, agentStarts) {
